...
import { CoopComercialData, Band } from '../types';
import { Plus, Trash2, ChevronDown, ChevronUp, AlertCircle } from 'lucide-react';
import Footer from './Footer';

interface Props {
  initialData: CoopComercialData;
  onSubmit: (data: CoopComercialData) => void;
  onBack: () => void;
}

const StepCoopComercialForm: React.FC<Props> = ({ initialData, onSubmit, onBack }) => {
  const [formData, setFormData] = useState<CoopComercialData>(initialData);
  ...
  // Handlers
  const handleChange = (field: keyof CoopComercialData, value: any) => {
    setFormData(prev => ({ ...prev, [field]: value }));
  };

  const handleNumberChange = (field: keyof CoopComercialData, valueStr: string) => {
    const val = valueStr === '' ? 0 : parseFloat(valueStr);
    handleChange(field, val); 
  };

  const handlePositiveNumberChange = (field: keyof CoopComercialData, valueStr: string) => {
    const val = valueStr === '' ? 0 : parseFloat(valueStr);
    ...
    <div className="flex flex-col min-h-full">
      <form onSubmit={handleSubmit} className="space-y-8 animate-fade-in flex-grow">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-semibold text-gray-800 mb-4 border-b pb-2 border-slate-200">DETALLE DE SU CONSUMO (Comercial)</h3>
...
export default StepCoopComercialForm;
